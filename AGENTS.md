...
Visual direction has moved **away from the Dreamcast/PS2-era retro look** referenced early in development, toward "uncanny realism suspended between the photographic and the obviously constructed". `docs/ART_DIRECTION.md`'s "Core principles" section already states this current direction explicitly and is not stale. `docs/VISUAL_LANGUAGE.md` was the document actually still describing and configuring the old retro-console target (title, "Central principle", faceted shading, 32-level colour posterisation, nearest-filtered signage) — reconciled 2026-09-24 as Stage 1 of `docs/REALISM_PASS_PLAN.md` (see that plan and `docs/VISUAL_REALISM_ROADMAP.md` for the fuller diagnosis and the staged path toward the current photographic-realism target).
