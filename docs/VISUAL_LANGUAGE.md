...
Zealot of Harperhay should look like photographs of Manchester reconstructed imperfectly inside an ambitious late-1990s or early-2000s console game. The style comes first from low-resolution photographic information, economical geometry, embedded lighting and a restricted palette. Post-processing is a restrained finishing layer, not the source of the retro character.
...
Wetness is represented with moderately reduced roughness plus sparse additive colour fragments. It deliberately does not use a continuous mirror plane: practical lights, shopfronts, the shelter and the pickup create broken streaks separated by dark asphalt.
...
Roads, paths, pavement, grass and the car park use repeated low-resolution colour textures with high roughness and no normal maps. Damp pavement variants break uniform repetition. Sparse coordinate-like road annotations introduce the measured/catalogued language of *The Spectres Are All Around Us*.
