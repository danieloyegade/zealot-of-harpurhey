# Sync Archive — Zealot of Harpurhey

Older entries moved out of `SYNC.md` (newest first, same order as the original log) so sessions don't have to read them at startup. Not required reading — search it (`grep -n` for a topic or asset name) when you need the history behind a decision. Add new archived entries at the top, below this header.
