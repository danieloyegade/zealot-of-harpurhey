...
---
## 2026-09-23 — Claude (pushed input/collision + Sterling Bike fixes to main)
**HEAD at session start:** `8bf80f0` (Record the Real Camera texture pass in SYNC.md), on branch `codex/three-issue-fixes`
**Did:**
- At Daniel's request, reviewed the two commits already sitting on `codex/three-issue-fixes` (`517aa1a` input/stale-focus/collision-hot-path fixes, `6692d64` Sterling Bike rear-cover geometry + materials pass) for gameplay impact and merge-readiness.
- Chased down the one open question left on `6692d64`: the SYNC.md entry for that commit flagged a black canvas at `?view=sterling-south` in the embedded browser preview, unresolved. Reproduced it, then reproduced the identical `GL_INVALID_FRAMEBUFFER_OPERATION: ... Attachment has zero size` warning on an unrelated, pre-existing view (`?view=park-to-dreams`) whenever the preview pane isn't actively displayed/composited. Confirmed it's a preview-tool artifact, not a regression introduced by the Sterling Bike commit.
- Confirmed `main`/`origin/main` were both still at `8bf80f0`, exactly where `codex/three-issue-fixes` branched from (no divergence), then fast-forwarded: `git push origin codex/three-issue-fixes:main`, followed by `git branch -f main origin/main` to update the local ref. `main` (local and remote) is now at `6692d64`.
**Left uncommitted (if any):** Nothing changed hands here — the push updated refs only, no checkout of `main` was needed, so the working tree (still on `codex/three-issue-fixes`, carrying the uncommitted Greek Gyros texture pass from earlier today) was left exactly as it was.
**Flagged:** None new.
**Next:** The Greek Gyros texture work is still uncommitted, awaiting Daniel's in-game review at `?view=greek-gyros` per the prior entry before it's committed.
**Open questions:** None.

---
## 2026-09-23 — Codex (photo-traced trolley geometry correction)
**HEAD at session start:** `6692d64` (Fix the Sterling Bike's lopsided rear cover and add a materials/texture pass), on `main`.
**Did:**
- Followed Daniel’s new supplied-photo brief, which explicitly supersedes the previous written-spec-only trolley. Read the creative constitution and relevant asset/technical docs; preserved the existing uncommitted shelter/trolley and Greek Gyros work.
- Rebuilt `build_trolley()` around measured pixel landmarks from the supplied 1818×1456 photograph. Each side is now one continuous bent tube with a short saddle under the basket’s rear and long, splayed runs down to the widely separated casters. Rebuilt the asymmetric basket, 27 thin ribs per side, 12 per end, horizontal bands and actual crossed-wire floor. Added open caster forks, grounded tyres and attached chain. Materials and lighting untouched.
- Added `reviewTrolleyGeometry.py` with separate validate/integrate stages. Inspected structural overlay on the photo, completely black orthographic side silhouette on white, and black three-quarter view before integration. Saved those renders, supplied reference and trace landmarks in `renders/trolley-geometry/`, plus standalone editable `preston-shopping-trolley.blend`.
- Replaced only the trolley hierarchy in the four existing shelter .blend files, exported standalone and combined GLBs (including the textured game model), copied these into `.runtime-public/`, and bumped the existing game loader cache key. No texture rebake. Updated `docs/assets/bus-shelter.md` to retire the previous conflicting geometry instructions.
- Validation: exported combined GLB preserves all 36 shelter meshes/transforms, 21 embedded image byte sequences and 11 material definitions exactly against the pre-session export. Verified 2 continuous frame sides, 54 side ribs, 37 floor wires and 4 tyres in the standalone export. `tsc --noEmit`, runtime asset validation (37 entries) and `git diff --check` passed. Results saved in `export-validation.json`.
**Left uncommitted (if any):** All this task’s geometry/source/export/render/doc/cache-key changes and this entry; pre-existing Greek Gyros, shelter texture/artwork/render work and unrelated untracked references untouched. No commit made.
**Flagged:** Photo governs over approximate written ratios/ASCII; hidden width and absolute scale are inferred from one image. Existing whole-shelter review PNGs predate this correction; current geometry evidence is `renders/trolley-geometry/`. CUA timed out twice. Isolated headless Chrome rendered the game loading placeholder without JS errors, but failed to complete a useful loaded-shelter capture; live in-game visual verification remains inconclusive. Closed that isolated browser. Local Vite server remains on port 5173.
**Next:** Review the black silhouette and photo overlay, then the refreshed `?intro=off&view=bus-shelter` preview in an ordinary browser.
**Open questions:** None.

---
## 2026-09-23 — Claude (bus-stop trolley rebuilt to a written spec, superseding the photo-matched pass below)
**HEAD at session start:** same session as the entry immediately below; no new commit landed between them.
**Did:**
- Daniel reviewed the photo-matched trolley from the entry below (scale fix, asymmetric basket, chrome + black handle) and judged it still wrong: "a generic tall shopping trolley," not the specific object he wanted. He gave a long, detailed written silhouette brief: long/low/shallow trapezoidal wire basket (~25-30% of total height, not ~40% as in the photos), an open A-frame undercarriage (two diagonal tubes per side, no axle bars, a large triangular negative space underneath, small casters), a thin metal handle (no plastic), and aged/dull galvanised steel rather than polished chrome.
- Flagged before building: this brief's proportions and construction don't match either photo I have (`Hires2.jpg`, where the basket is closer to ~40% of total height, and a clean product photo, where the handle is black plastic and the chassis is a conventional 4-wheel frame). Asked Daniel directly whether to wait for a new photo, follow the written brief as authoritative, or keep the photos as ground truth and only apply the parts of his feedback they support. **He chose: follow the written brief as-is**, explicitly over the photos.
- Rebuilt `build_trolley()` in `blender/scripts/createBusShelter.py` from scratch against that brief, not the photos: shallow basket capped at ~0.28 m depth (rim 1.02 m, front-bottom 0.74 m) instead of the previous ~0.84 m depth that ate the whole undercarriage; removed the front/rear axle bars and rebuilt the undercarriage as two long diagonal tubes per side running straight from the basket's own rear-bottom/front-bottom corners to small casters (added a `radius` parameter to `add_wheel()`, default unchanged, used at 0.052 vs the previous 0.09); replaced the black-plastic handle with a thin metal loop; thinned every wire/tube radius further (rim 0.018→0.013, side wire 0.007→0.0045, frame tube 0.023→0.016) for a more delicate, skeletal read. Renamed materials again: `MAT_Trolley_GalvanisedSteel` (duller, roughness 0.50/metallic 0.78, replacing the shinier chrome from the entry below) for the whole structure including the handle, and repurposed the black-plastic material slot into `MAT_Trolley_WornBracket` (darker, less metallic) for the wheel forks only, as the one deliberate material break.
- Rebuilt through the same real pipeline as before (`createBusShelter.py` then `createBusShelterTextured.py -- --bake-cache /tmp/bus-shelter-bake`, ~6 min with the cache), copied the refreshed GLBs into `.runtime-public/`, and validated against the brief's own checklist using Blender review renders from multiple angles before touching the production asset: shallow long basket ✓, thin wire ✓, open triangular gap under the basket ✓, diagonal tubular legs clearly visible ✓, small casters ✓, no axle-bar chassis ✓, no black/plastic anywhere ✓. Confirmed live in `?view=bus-shelter` afterward.
- Updated `docs/assets/bus-shelter.md`'s "Trolley geometry and materials" section to record that this geometry is now intentionally built to Daniel's written spec rather than the photographs, so a future session doesn't "fix" it back toward the photos without that being a deliberate call.
**Left uncommitted (if any):** Same files as the entry below, now further modified: `blender/scripts/createBusShelter.py`, `docs/assets/bus-shelter.md`, the regenerated `.blend` sources, GLBs, and `renders/bus-shelter*/`. `src/world/createWorld.ts` (scale/collision fix from the entry below) is unaffected by this pass. Pre-existing concurrent Greek Gyros work untouched, as noted below.
**Flagged:** No baked wear/grime pass, same caveat as before — the material is a flat "aged galvanised" constant, not per-pixel oxidation/rubbed-edge variation. The trolley's proportions and construction now deliberately diverge from both photos in this session's context; if that was a one-off creative call rather than a standing preference, say so explicitly next time, since nothing in the geometry itself will signal it was chosen over photographic evidence.
**Next:** If the result still isn't right, the next step should be a fresh, explicitly-supplied reference image (photo or sketch) of the specific trolley Daniel has in mind, rather than another round of text-described adjustments — that's what caused the back-and-forth this session.
**Open questions:** None.

---
## 2026-09-23 — Claude (bus-stop trolley scale, geometry and material fixes)
**HEAD at session start:** `6692d64` (Fix the Sterling Bike's lopsided rear cover and add a materials/texture pass), on branch `main`
**Did:**
- Daniel flagged the trolley at the bus stop as looking like a "toy" with wrong proportions. Traced it to `applyBusShelterGeometryPolicy()` in `src/world/createWorld.ts`: `preston-busstop-textured.glb` carries the shelter (`PRESTON_BUS_SHELTER`) and the modelled trolley (`PRESTON_SHOPPING_TROLLEY`) as sibling roots authored at the same real-world scale (confirmed by loading the combined GLB standalone in three.js — at 1:1 they match the reference photo). Runtime code scaled only the shelter root by `BUS_SHELTER_SCALE` (1.3x) and left the trolley at authored size, so it read ~30% undersized next to the enlarged shelter/bench.
- Scaled `PRESTON_SHOPPING_TROLLEY` by the same `BUS_SHELTER_SCALE` alongside the shelter, and scaled the trolley's collision clearance (`maxZ: marker.z + 1.85 * BUS_SHELTER_SCALE` in `addBusShelter`) to match, since its footprint grows with it.
- Daniel then compared the scaled-but-otherwise-unchanged trolley against a clean product photo and flagged two more concrete defects: the basket tapered symmetrically on all four sides (should be a short near-vertical rear wall by the handle and a long sloped nose wall, so trolleys nest), and the handle read red when it should be black. Rebuilt `build_trolley()`'s basket in `blender/scripts/createBusShelter.py` with independent rear (`rx0/rx1/ry/rz`) and front/nose (`fx0/fx1/fy/fz`) bottom corners instead of one shared bottom rectangle, so the rim, corner posts, side wire mesh and horizontal ridge wires all follow the correct lopsided profile. The "red handle" turned out to be a misread of `Hires2.jpg`: the shelter's red `BUSSTOP_BenchRail` shows through the trolley's open wire frame from behind and looks like part of the trolley at that resolution. Fixed `create_materials()`: `MAT_Trolley_Chrome` (brighter, more metallic, was dull painted-metal grey), `MAT_Trolley_BlackPlastic` for the handle grip only (was an unrelated near-black green-grey, not red), `MAT_Trolley_Tyre` (unchanged rubber, slightly rougher). Dropped the `_PLACEHOLDER` suffixes since these are now the intended materials.
- Regenerated the trolley/shelter Blender sources and every runtime GLB by rerunning `createBusShelter.py` (fast, geometry-only) then `createBusShelterTextured.py -- --bake-cache /tmp/bus-shelter-bake` (the actual pipeline that produces `preston-busstop-textured.glb`, the one the game loads) twice — once for materials only, once more after the basket-shape fix. The bake cache made the second full run ~4.5 minutes instead of ~15 since shelter geometry/UVs didn't change. Copied the refreshed GLBs into the gitignored `.runtime-public/` so local preview reflects the fix without a full `npm run assets:prepare`.
- Verified at each step: Blender review renders (side-profile silhouette) confirmed the new basket shape and black handle before touching the production asset; a standalone three.js viewer (temporary page under `.runtime-public/`, removed after) confirmed the exported GLB's shape from an orbit camera; the live game at `?view=bus-shelter` confirmed the final in-game look, chrome catching the shelter lights correctly. `tsc --noEmit` is clean throughout.
- Updated `docs/assets/bus-shelter.md` with a new "Trolley geometry and materials" section explaining the asymmetric-basket rationale and the material renames, and corrected its "Asset status" summary.
- Note: there's a second, unrelated "abandoned shopping trolley" prop (`addShoppingTrolley()` in `createWorld.ts`, built from raw boxes/wireframe) placed near the East Shops corner (31.9, 20.9) and near (-7.1, -30.1) — not at either `BUS_STOPS` marker, so out of scope here, but it's genuinely crude (flat wireframe box for the basket, a floating flat bar for the handle) and would read just as toy-like if a player got close.
**Left uncommitted (if any):** `src/world/createWorld.ts` (scale/collision fix), `blender/scripts/createBusShelter.py` (basket shape + materials), `docs/assets/bus-shelter.md`, the regenerated `blender/source/bus-shelter/*.blend` files, the refreshed GLBs under `public/assets/models/bus-shelter/` and `renders/bus-shelter*/`, and this entry. Pre-existing concurrent Greek Gyros work (`config/building-textures.json`, `docs/assets/greek-gyros.md`, `public/assets/models/greek_gyros.glb`, `blender/scripts/greekGyrosTextures.py`, `blender/source/runtime-untextured/greek_gyros.glb`, `blender/source/textures/greek-gyros/`) and the untracked `.playwright-mcp/`, `references/architecture/infrastructure:objects/Busses/`, `references/screenshots/dreams-01-street-view.png`, `renders/greek-gyros-textures/` predate this session and were left untouched.
**Flagged:** No baked wear/grime pass on the trolley (unlike the shelter's full weathering bake) — it's now correctly shaped and coloured but still reads clean/unweathered up close. The crude street-dressing trolley noted above.
**Next:** If Daniel wants the trolley to fully match the shelter's fidelity, it needs a proper UV-atlas weathering bake (reusing `surfaceWeathering.py`, as `createBusShelterTextured.py` already does for the shelter) rather than flat material constants. Separately, if the East Shops/street-dressing trolley should be fixed too, `addShoppingTrolley()` in `createWorld.ts` needs a real rebuild — it's built from primitives, not a scale mismatch.
**Open questions:** None.
...
---
## 2026-09-23 — Codex (Greek Gyros surface textures)
**HEAD at session start:** `8bf80f0` (Record the Real Camera texture pass in SYNC.md)
**Did:**
- Completed building-texture plan Task 10 for Greek Gyros. Added five PBR sets (warm white enamel, measured blue enamel, darker secondary-fascia blue, dark coated metal and stainless steel), the texture contract, preserved untextured runtime GLB, runtime map profiling and a `textured-20260923` cache key. The shipped GLB now embeds 15 maps, is 3.62 MB and preserves all nodes/anchors.
- Used built-in ImageGen for one blue-enamel source. The script discards its colour and reduces a mirrored/seamless luminance field to 3.5% contrast over the measured photographic blue; exact provenance is in `blender/source/textures/greek-gyros/validation.json`. Signage remains deferred and glass/fixture-lens placeholders remain runtime-controlled.
- Updated `docs/assets/greek-gyros.md`. `npm test` passes (18 tests), `npm run build` passes, `git diff --check` passes, and public/runtime/dist GLB hashes match. Checked `?intro=off&view=greek-gyros` in game: mapped surfaces and counter practical render, with no browser warnings/errors.
**Left uncommitted (if any):** All Greek Gyros texture work above plus this entry. Existing unrelated `.playwright-mcp/`, bus references and Dreams screenshot remain untouched.
**Flagged:** Only ~674 MiB free after the development/production asset copies and build. The secondary menu strip has no uncontaminated colour patch (lettering and night glare cover it), so its darker blue is explicitly inferred from the measured main/lower blue. The steel set is shared by apron, counters and fittings; a diamond-plate normal requires a future material split or it would incorrectly affect every stainless surface.
**Next:** Daniel can review the open `?view=greek-gyros` tab. The next distinct pass is signage/Greek flags/menu artwork, or Task 11 (Village Books) if signage stays deferred.
**Open questions:** None.
...
**Flagged:** The new visual has no menu, so entry is now automatic after loading. Browser autoplay policy means music/ambience still begins on the player's first key or pointer input. The existing visible *Harpurhey* versus codebase *Harpurhey* discrepancy remains.
...
**Open questions:** Harpurhey or Harpurhey? And should the phone treatment be a portrait crop of the plate, or the landscape plate with a real menu underneath it?
...
- **Voice 03 (hand):** the Water Brush script in the `--intro-hand` token.
  - A giant handwritten "Harpurhey" is screen-printed behind the classical title once the city is assembled (7.5% ivory plus a misregistered 3.5% amber pass, via the `#intro-ink` SVG filter in `index.html`).
...
- Added marginalia, written by `src/ui/title/marginalia.ts` into `data-intro-slot` elements in `index.html`:
  ...
  - a hairline site plan drawn from `worldLayout.ts`, above a Harpurhey → Promised Land route and Service 01 timetable
...
- Added `src/ui/title/riderRecord.ts`, which reads `localStorage['zealot-of-harpurhey:rider-record']` over first-night defaults and writes only `entries`, plus `src/delivery/firstDelivery.ts` as data.
...
**Flagged:**
- The title uses **Harpurhey**, the canonical spelling in `AGENTS.md`. Daniel typed "Harpurhey" in his request, so this needs confirming.
...
**Open questions:** Harpurhey or Harpurhey on the title card? Keep the imprint lines?
...
**Did:** Looked into "game isn't loading on localhost". No code changed. No dev server was running. Started `zealot-dev` from `.claude/launch.json`. The game loads at `http://localhost:5173/zealot-of-harpurhey/` with no console errors and no failed requests (110 resources). `tsc --noEmit` passes.
**Left uncommitted (if any):** Only this entry, on top of the existing batch.
**Flagged:** The Vite `base` is `/zealot-of-harpurhey/`, so the bare `localhost:5173/` URL doesn't show the game. The checkout folder is still `zealot-of-harpurhey`, even though the 2026-09-11 entry says the folder was renamed.
**Next:** None.
**Open questions:** None.
...
- **`docs/ART_DIRECTION.md`:** appended these sections:
  ...
  - transport: the Harpurhey ⇄ The Promised Land bus, the bicycle system, bikes as cultural objects, and fashion as social worldbuilding.
...
- **Spelling:** Daniel's text used "Harperhay". I normalised it to canonical "Harpurhey".
...
**Open questions:**
- Are the existing Bus Stop A/B the Harpurhey and The Promised Land stops, or is The Promised Land a new location beyond the current map?
...
**Checked in game, at dev-view distance only:** once the "localhost hang fix" entry above landed, `http://localhost:5173/zealot-of-harpurhey/?view=bus-shelter` rendered.
...
## 2026-09-11 — Claude (4)
**HEAD at session start:** `ed92801` (Rename harperhay to harpurhey across assets, scripts, docs, and config)
...
## 2026-09-11 — Claude (2)
**HEAD at session start:** `d4b6dff` (Add hero location Blender assets and expand world/environment systems)
**Did:** Resolved the naming question both prior entries left open. Daniel's decision: the project name is **Harpurhey** (not `harperhay`, not the real-world `harpurhey`) — a deliberate fictional spelling distinct from the real Manchester district, consistent with this being a fictional collage city. Renamed every asset file, script path, doc reference, `package.json` name, Vite `base` path, and page `<title>` from `harperhay`/`harpurhey` to `harpurhey` (case-preserved). Renamed the top-level project directory from `zealot-of-harpurhey` to `zealot-of-harpurhey`. Also committed the 155-file working-tree batch from the prior sessions (Coral/Cass Art/Gulliver's/Hive/MCR1/Renee assets, references reorg) as its own commit before doing the rename, so the two changes stay legible in history.
...
**Next:** If any external references to the old path exist (deploy config, bookmarks, other clones), they'll need updating to `/zealot-of-harpurhey/` and the new directory name.
