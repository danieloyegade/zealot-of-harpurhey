...
## 2026-09-12 — Claude (movement & camera)
**HEAD at session start:** `85c91e5` (perf: stabilise simulation timing and reduce dynamic lighting) — a two-day-old base. Merged `origin/main` (`4562e6d`) mid-session once I realised how far behind I was; see Flagged.
**Did:** Two rounds of movement/camera work, each reviewed then fixed.

Round 1 (camera and movement ergonomics):
- Camera occlusion: `findSegmentObstruction` in `collision.ts` segment-tests the obstacle boxes so the camera stops in front of geometry instead of sitting inside the shopfronts. `CollisionObstacle` gained `height` for this.
- Player figure hides below 1.7 m camera distance (hysteresis to 1.95 m) so a wall-pinned camera does not fill the screen with the character.
- Pointer lock on click (drag retained as fallback), `Q`/`E` yaw and `R`/`F` pitch for mouseless play, wheel zoom 3.4–10.5 m, orbit sensitivity normalised to screen height.
- Single smoothed anchor drives both camera position and look target, so the player no longer slides around the frame under acceleration. FOV 58, widening to 64 at a run.
- Movement acceleration 8/11 → 16/20 with a zero-snap on stop; running is now the default pace with `Shift` to walk; velocity reconciled against actual displacement so walls drop the blocked component.

Round 2 (fixing round 1, after re-reviewing it):
- **Removed the auto-follow I had just added — it was a feedback loop.** Movement is camera-relative, so easing camera yaw toward the direction of travel rotates the movement basis, which rotates travel further. Measured: holding strafe traced a closed circle, full rotation roughly every 2 s, returning to the start point. It also did nothing useful when running forward, where facing already equals camera forward. Replaced with an explicit one-shot recenter on `C`.
- Occlusion is now a hard constraint applied to the final camera position rather than a target eased toward, so pulling in is immediate and easing out falls out of the follow lerp. Previously both directions lerped at the same rate and the camera spent ~100 ms inside any wall it had just detected.
- Street furniture is solid: 20 streetlight columns, 4 utility cabinets, 2 trolleys and 2 bike docks now push footprints (71 obstacles total, up from 43). They carry `occludesCamera: false` — they block movement but must not yank the camera in as the player walks past a lamppost.

**Left uncommitted (if any):** None.
**Flagged:**
- **I started from a stale base and duplicated your workflow.** Not seeing `SYNC.md`/`AGENTS.md` on my 2-day-old branch, I concluded no coordination workflow existed and built a parallel one (`docs/STATUS.md`, `docs/AGENT_WORKFLOW.md`, plus my own `CLAUDE.md`/`AGENTS.md`). That is exactly the parallel duplicate `AGENTS.md` warns against. On merging I deleted all of mine and kept yours. The root cause was mine: I never fetched `origin/main` before starting. Worth noting the same trap exists for any session that branches from an old base.
- Merge reconciliation: I dropped my own single-pass depenetration in favour of your `resolveCircleOverlaps`, which iterates to a fixed point and is strictly better. `moveCircleWithCollisions` now applies full movement then calls it, which resolves to exact contact rather than stopping a frame short (verified: player rests at a building face + exactly the 0.38 m radius).
- Interior collision added on main (Renee, Cass Art, Coral, Greek Gyros) needed heights for the camera test. Walls use `location.height`; Coral's betting counters are 1.1 m; Greek Gyros 3.6 m. Worth a sanity check from whoever authored them.
- Railings in `addHeroStreetEnvironmentKit` are still walk-through. I did not add footprints because that module places them and I would have been guessing positions. They are visually prominent in most street shots.
- **This world can no longer be simulated in a software-rendered browser.** At 247k triangles / 1945 draw calls, swiftshader frames take ~666 ms, which exceeds `FixedStepClock`'s 250 ms extreme-gap threshold, so gameplay never advances and the player appears frozen. That is correct engine behaviour, not a bug, but it means headless verification in CI-style environments needs a non-rendering path. I verified this session's changes by driving the real `PlayerController`/`ThirdPersonCamera` update loop through a temporary harness page with no renderer, then deleted it.
**Next:** Gamepad and touch input remain absent — touch devices have no movement input at all. Railing footprints per above. `invertPitch`/`orbitSensitivity` exist on the camera but nothing exposes them to a player yet.
**Open questions:**
- Running is now the default pace with `Shift` to walk, on the grounds that the world takes ~53 s to cross at walking speed. I took that call myself; it is a one-line flip in `PlayerController.update` if you disagree.
- Is mobile/touch in scope at all? It changes whether the input layer needs restructuring now or later.
